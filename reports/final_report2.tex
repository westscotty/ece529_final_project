...
The function \texttt{calcOpticalFlowPyrLK\_numpy} is the custom NumPy-based implementation of the pyramidal Lucas-Kanade method, while \texttt{calcOpticalFlowPyrLK} uses OpenCV's optimized implementation. Both functions follow the same basic structure but differ in implementation. The NumPy version directly computes gradients and solves the flow equations, while OpenCV uses highly optimized C++ functions for these operations.

% \subsubsection{Point Tracking}
At each frame, the feature points are tracked from their position in the previous frame to their new position in the current frame. The \texttt{validate\_points} function ensures that only valid points are kept (i.e., points that are successfully tracked). It checks the tracking status (\texttt{st}) and the error (\texttt{err}) against predefined thresholds to filter out unreliable tracks.
...
\end{itemize}
The blue points are simply the Shi-Tomasi corners. The green points are considered good points tracked from one the previous frame to the current frame, and the red points are new corners added to the list of viable points for the optical flow calculations based on the reinitalization criteria. Figures \ref{fig:sample_1} through \ref{fig:sample_4} show a 4-frame sequence of corners being detected, and persisting over several frames, with many corners falling away and new corners being initizalized.
\bigskip
...
Throughout the tracking process, several metrics are calculated to evaluate the performance of the optical flow algorithm:
\begin{itemize}
    \item Mean Absolute Error (MAE) between the tracked points and the true positions (or ground truth).
    \item Peak Signal-to-Noise Ratio (PSNR) and Structural Similarity Index (SSIM) are calculated to assess the image quality after tracking.
    \item Precision and Recall are computed to measure the accuracy of point matching between the detected corners in the current frame and the previous frame.
\end{itemize}
\bigskip
...
\begin{lstlisting}[style=python, caption={Full Monte Carlo Analsys File}]
    import numpy as np
...
\begin{lstlisting}[style=python, caption={Full Shi-Tomasi Corners File}]
    import cv2
    import numpy as np
...
\begin{lstlisting}[style=python, caption={Full Image Operations File}]
    import cv2
    import numpy as np
    import sys
...
\begin{lstlisting}[style=python, caption={Full Lucas Kanade Optical Flow File}]
    import cv2
    import os
    import numpy as np
    from tqdm import tqdm
...
\begin{lstlisting}[style=python, caption={Full Video Processing Utilities File}]
    import cv2
    import numpy as np
    from tqdm import tqdm
...
\begin{lstlisting}[style=python, caption={Full Plotting Utilities File}]
    import numpy as np
    import cv2
    import matplotlib.pyplot as plt
...
\begin{lstlisting}[style=python, caption={Full Other Utilities File}]
    import numpy as np
